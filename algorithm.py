...
def dijkstra(node):
    d[node][1] = 0
    pq = []
    heappush(pq, (0, node, 1, "0"))

    while pq:
        cost, node, status, st = heappop(pq)

        if cost > d[node][status]: continue

        if status == size - 1 and v[node][0] != 0:
            cur_cost = cost + v[node][0]
            if result["dist"] > cur_cost:
                result["dist"] = cur_cost
                result["st"] = st
        ...
        for n_node in range(n):
            ...
            if d[n_node][n_status] > n_cost:
                ...
                heappush(pq, (n_cost, n_node, n_status, f"{st} {str(n_node)}"))
dijkstra(0)
